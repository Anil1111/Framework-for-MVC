...
WHEN MATCHED THEN 
	UPDATE SET [FirstName] = Source.[FirstName], [MiddleName] = Source.[MiddleName], [CustomerTypeId] = CT.[CustomerTypeId],
		[LastName] = Source.[LastName], [BirthDate] = Source.[BirthDate], [ActivityContextId] = @ActivityContextId_Staging
-- Insert 
WHEN NOT MATCHED BY TARGET THEN 
	INSERT ([CustomerKey], [FirstName], [MiddleName], [LastName], [BirthDate], [CustomerTypeId], [CreatedActivityId]) 
		Values (Source.[CustomerKey], Source.[FirstName], Source.[MiddleName], Source.[LastName], Source.[BirthDate], CT.[CustomerTypeId], @ActivityContextId_Staging)
;
