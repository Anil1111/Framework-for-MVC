...
namespace Framework.Application
{
    ...
    public class ConfigurationManagerUwp : ConfigurationManagerSafe
    {
        ...
        public ConfigurationManagerUwp(ApplicationTypes appType) : base(Directory.GetCurrentDirectory())
        {
            ...
            RootFullPath = RootFolder.AddLast(@"\") + (ApplicationType == ApplicationTypes.Native ? FileNames.AppJson : FileNames.WebJson);
            AppSettingsFullPath = AppDataFolder.AddLast(@"\") + FileNames.AppSettingsJson;
            ConnectionStringsFullPath = AppDataFolder.AddLast(@"\") + FileNames.ConnectionStringsJson;
            Load();
        }
        ...
        public void Load()
        {
            ...
            configBuilder = new ConfigurationBuilder().AddJsonFile(AppSettingsFullPath, optional: true);
            ...
            configBuilder = new ConfigurationBuilder().AddJsonFile(ConnectionStringsFullPath, optional: true);
            configuration = configBuilder.Build();
            foreach (var Item in configuration.GetChildren())
            {
                ConnectionStrings.Add(Item.Key, Item.Value);
            }
        }
    }
}